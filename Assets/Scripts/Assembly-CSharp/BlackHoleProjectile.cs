...
public class BlackHoleProjectile : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (collapsing)
		{
			...
			foreach (EnemyIdentifierIdentifier enemyIdentifierIdentifier in array)
			{
				...
				enemyIdentifierIdentifier.eid.DeliverDamage(enemyIdentifierIdentifier.gameObject, Vector3.zero, enemyIdentifierIdentifier.gameObject.transform.position, 100f, false, 0f);
				if (!enemyIdentifierIdentifier.eid.exploded)
				{
					...
					scalc.HitCalculator("", "", "", true, base.gameObject);
				}
			}
	...
	private void ShootRandomLightning()
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			{
				GameObject gameObject = Object.Instantiate(lightningBolt, base.transform.position, base.transform.rotation);
	...
	private void ShootTargetLightning()
	{
		if (shootList.Count != 0)
		{
			...
			{
				GameObject gameObject = Object.Instantiate(lightningBolt2, base.transform.position, base.transform.rotation);
				LineRenderer component = gameObject.GetComponent<LineRenderer>();
				component.SetPosition(0, base.transform.position);
				component.SetPosition(1, shoot.transform.position);
				shoot.hitter = "secret";
				shoot.DeliverDamage(shoot.GetComponentInChildren<EnemyIdentifierIdentifier>().gameObject, Vector3.zero, shoot.transform.position, 1f, false, 0f);
				if (shoot.dead)
				{
					list.Add(shoot);
					Rigidbody[] componentsInChildren = shoot.GetComponentsInChildren<Rigidbody>();
					Rigidbody[] array = componentsInChildren;
					foreach (Rigidbody item in array)
					{
						caughtList.Add(item);
					}
				}
