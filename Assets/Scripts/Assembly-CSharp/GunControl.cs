...
public class GunControl : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (currentWeapon == null && slots[currentSlot - 1].Count > 0)
		{
		...
		shud = GameObject.FindWithTag("Player").GetComponentInChildren<StyleHUD>();
	}
	...
	private void Update()
	{
		if (activated)
		{
			...
			else if (Input.GetKeyDown(inman.inputs["LastUsedWeapon"]) && lastUsedSlot != 69)
			{
				...
				{
					SwitchWeapon(lastUsedSlot, slots[lastUsedSlot - 1], true);
				}
			}
	...
	{
		if (!activated || allWeapons.Count == 0)
		{
			return;
		}
		for (int i = 0; i < allWeapons.Count; i++)
		{
			...
			else if (allWeapons[i].activeSelf)
			{
				...
				if (weaponFreshnesses[i] < 10f)
				{
					...
					shud.freshWeaponObj.SetActive(false);
				}
				else
				{
	...
	public void SwitchWeapon(int target, List<GameObject> slot, bool lastUsed = false)
	{
		...
		shud.freshWeaponObj.SetActive(false);
		int num = target;
		Debug.Log("Current Slot: " + currentSlot + ". Current Variation: " + currentVariation);
		if (lastUsed)
		{
		...
		if (!noWeapons)
		{
			...
			if (weaponFreshnesses[allWeapons.IndexOf(currentWeapon)] > 15f)
			{
				...
				shud.freshWeaponObj.SetActive(true);
			}
			aud.Play();
