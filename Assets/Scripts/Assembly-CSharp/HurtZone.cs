...
public class HurtZone : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		foreach (EnemyIdentifier hurt in hurtList)
		{
			...
			if (num <= 0f)
			{
				...
				hurt.DeliverDamage(hurt.gameObject, Vector3.zero, hurt.transform.position, damage / 2f, false, 0f);
				if (burn)
				{
	...
	private void OnTriggerEnter(Collider other)
	{
		...
		else
		{
			...
			component.eid.DeliverDamage(other.gameObject, Vector3.zero, other.transform.position, damage / 2f, false, 0f);
			if (burn)
			{
